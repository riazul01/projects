...
const ProjectCard = ({data}) => {
    return (
        <div className="h-[320px] w-full flex flex-col items-center justify-center bg-[#111] border-[1px] border-[#000] rounded-[1.2rem] overflow-hidden">
            <Link to={`/details/${data.id}`} className="relative w-full h-[80%] overflow-hidden">
                ...
                <div className="absolute bottom-[0.8rem] left-[1rem] flex items-center gap-[0.2rem] tags">
                    {data.tags.map((tag) => {
                        return <span className={`px-[0.6rem] py-[0.2rem] text-[0.9rem] font-[500] rounded-[1.2rem] ${tag}`}>{tag}</span>
                    })}
                </div>
            </Link>
            <div className="relative w-full h-[20%] flex items-center">
                <a href={data.site} className="project-title ps-[1rem] text-[1.1rem] font-[500]" target="_blank" rel="noreferrer">{data.title}</a>
